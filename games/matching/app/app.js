	...
	Draggable.create('.dragItem', {
		...
		onDragEnd: function() {
			...
			$.each(dropTargets, function(idx, spot){
				var spotID = spot.id,
						pos = $(spot).position(),
						diffTop = dragThing.offsetTop + pos.top,
						diffLeft = dragThing.offsetLeft + pos.left;

				if (spotID === dragID) {
					if (dragThing.hitTest(spot, '10%')) {
						TweenMax.to('.correct', .3, {
							scale: 1.5,
							autoAlpha: 1,
							onComplete: function() {
						...
						TweenMax.to(dragThing.target, .5, {
							...
							onCompleteParams: [dragThing, spot]
						});
					}else {
						TweenMax.to('.tryAgain', .3, {
							scale: 1.5,
							autoAlpha: 1,
							onComplete: function() {
...
function hideMatches(dragItem, targetItem) {
	totalHit++;
	TweenMax.to([dragItem.target, targetItem], .5, {
		autoAlpha: 0,
